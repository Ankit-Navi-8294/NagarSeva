...
def run_sla_watchdog():
    ...
    try:
        ...
        # Use FieldFilter if available (firebase-admin >= 6.x), else fallback to old syntax
        if HAS_FIELD_FILTER:
        ...
        for doc in query:
            ...
            if hours_elapsed > target_hours:
                ...
                action_msg = (
                    f"Escalated issue {doc.id} (Severity {severity}). "
                    f"Elapsed: {hours_elapsed:.1f}h / Target: {target_hours}h"
                )
                actions_taken.append(action_msg)

                # 2. Mock FCM Push Notification
                print(
                    f"[FCM Mock] Sending push to reporter "
                    f"{issue.get('reporter_uid')}: "
                    f"'Your issue has been escalated to senior authorities.'"
                )
