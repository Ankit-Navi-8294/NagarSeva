...
def get_embedding(text: str) -> list:
    ...
    if GEMINI_API_KEY == "YOUR_GEMINI_API_KEY" or not text.strip():
...
def run_duplicate_merger():
    ...
    try:
        ...
        # Use FieldFilter if available, else fallback to old syntax
        if HAS_FIELD_FILTER:
            query = issues_ref.where(filter=FieldFilter("status", "in", open_statuses)).stream()
        else:
        ...
        for i, issue1 in enumerate(issues):
            ...
            for j in range(i + 1, len(issues)):
                ...
                desc1 = (issue1.get("description", "")
                         or issue1.get("ai_classification", {}).get("auto_description", ""))
                desc2 = (issue2.get("description", "")
                         or issue2.get("ai_classification", {}).get("auto_description", ""))
                ...
                if similarity >= SIMILARITY_THRESHOLD:
                    ...
                    msg = (
                        f"Merged {secondary['_id']} into {primary['_id']} "
                        f"(Distance: {distance:.1f}m, Similarity: {similarity:.2f})"
                    )
                    actions_taken.append(msg)

                    # Mock FCM Notification
                    print(
                        f"[FCM Mock] Notifying {secondary.get('reporter_uid')}: "
                        f"'Your report was merged with an existing issue nearby. Upvotes combined!'"
                    )
